	...
	reloadWatcher.watcher = chokidar
		.watch(
			[
				...
				join(app.getAppPath(), 'SelectBluetoothDevice'),
			],
			{
				ignored: /[/\\]\./,
				persistent: true,
			...
			if (reloadWatcher.ready) {
				...
				reloadWatcher.debouncer = setTimeout(async () => {
					...
					electronCapacitorApp
						.getSelectBluetoothDeviceWindow()
						.webContents.reload();
...
export class ElectronCapacitorApp {
	...
	async init(): Promise<void> {
		...
		this.SelectBluetoothDeviceWindow.loadFile(
			'./SelectBluetoothDevice/SelectBluetoothDevice.html',
		);
