...
export class ConfigGpioComponent implements OnInit {
	...
	pinchanged(pinid: number) {
		this.ngZone.run(() => {
			this.select.value =
				GPIOPinType[this.gpio.getTempGPIOPins().get(pinid).getPinType()];
		});
	}
	...
	pinTypeChanged(event) {
		this.gpio
			...
			.setPinType(GPIOPinType[<string>event.detail.value]);
		this.gpio
			.getTempGPIOPins()
			.get(+this.selectedpin)
			.getPinConfigValue()
	...
	pwmRatioChanged(event) {
		this.gpio
			...
			.get(+this.selectedpin)
			.getPinConfigValue()
			.setUint8(2, (event.detail.value * 254) / 100);
	}
	...
	inputTypeChanged(event) {
		this.gpio
			...
			.get(+this.selectedpin)
			.getPinConfigValue()
			.setUint8(0, GPIOInputPinType[<string>event.detail.value]);
	}
	...
	outputTypeChanged(event) {
		this.gpio
			...
			.get(+this.selectedpin)
			.getPinConfigValue()
			.setUint8(0, GPIOOutputPinType[<string>event.detail.value]);
	}
