	...
	implements GPIOInterface
{
	...
	private pendingCommandResponseResolver:
		| ((commandResponse: ProteusCommand) => void)
		| null = null;
	...
	handlerx(data: DataView): void {
		switch (data.getUint8(0)) {
			case ProteusHeader.RF_HEADER_TYPE_DATA:
				this.logDataReceived(
					'LogMessages.DataReceived',
					undefined,
					data.buffer.slice(1),
				);
				break;
			case ProteusHeader.RF_HEADER_TYPE_CMD:
				switch (data.getUint8(1)) {
					case ProteusCommand.CMD_GETSTATE_CNF: {
						...
						this.logRemoteCommand(
							ProteusCommand[ProteusCommand.CMD_GETSTATE_CNF],
							undefined,
							data.buffer,
						);
						break;
					...
					case ProteusCommand.CMD_GPIO_REMOTE_READCONFIG_CNF: {
						...
						this.logRemoteCommand(
							ProteusCommand[ProteusCommand.CMD_GPIO_REMOTE_READCONFIG_CNF],
							undefined,
							data.buffer,
						);
						var blockIdx = 2;
						while (blockIdx < data.buffer.byteLength) {
							...
							if (blockSize > 2) {
								...
								let pinValue = new DataView(
									data.buffer.slice(blockIdx + 3, blockIdx + 3 + blockSize - 2),
								);
								switch (pinValue.byteLength) {
									...
									case 0x1:
										gpiopin
											.getPinConfigValue()
											.setUint32(0, pinValue.getUint8(0), true);
										break;
									case 0x3:
										gpiopin
											.getPinConfigValue()
											.setUint32(
												0,
												pinValue.getUint16(0) + (pinValue.getUint8(2) << 16),
												true,
											);
										break;
								}
						...
						this.proteusGPIO.resetTempGPIOPins(
							Array.from(this.proteusGPIO.getTempGPIOPins().values()),
						);
						break;
					}
					case ProteusCommand.CMD_GPIO_REMOTE_WRITECONFIG_CNF: {
						...
						this.logRemoteCommand(
							ProteusCommand[ProteusCommand.CMD_GPIO_REMOTE_WRITECONFIG_CNF],
							undefined,
							data.buffer,
						);
						var blockIdx = 2;
								...
								case 0x0:
									this.proteusGPIO
										...
										.set(
											gpioid,
											GPIOPin.copy(
												this.proteusGPIO.getTempGPIOPins().get(gpioid),
											),
										);
									changedpins.push(
										this.proteusGPIO.getTempGPIOPins().get(gpioid),
					...
					case ProteusCommand.CMD_GPIO_LOCAL_WRITE_IND: {
						...
						switch (data.getUint8(1)) {
							case ProteusCommand.CMD_GPIO_REMOTE_READ_CNF: {
								this.logRemoteCommand(
									ProteusCommand[ProteusCommand.CMD_GPIO_REMOTE_READ_CNF],
									undefined,
									data.buffer,
								);
								break;
							...
							case ProteusCommand.CMD_GPIO_LOCAL_WRITE_IND: {
								this.logRemoteCommand(
									ProteusCommand[ProteusCommand.CMD_GPIO_LOCAL_WRITE_IND],
									undefined,
									data.buffer,
								);
								break;
					...
					case ProteusCommand.CMD_GPIO_REMOTE_WRITE_CNF: {
						...
						this.logRemoteCommand(
							ProteusCommand[ProteusCommand.CMD_GPIO_REMOTE_WRITE_CNF],
							undefined,
							data.buffer,
						);
						var blockIdx = 2;
				...
				if (this.pendingCommandResponseResolver) {
					this.pendingCommandResponseResolver(data.getUint8(1));
				}
				break;
				...
				if (fragmentNumber > fragmentsCount) {
					...
					if (fragmentNumber == fragmentsCount) {
						...
						for (let i = 0; i < fragmentsCount - 1; i++) {
							if (
								fragmentsCount !=
									(this.proteusHighThroughput
										.getFragmentsDataByIndex(i)
										.getUint8(2) &
										0x0f) ||
								data.getUint8(1) !=
									this.proteusHighThroughput
						...
						combinedData = combinedData.concat(
							Array.from(new Uint8Array(data.buffer.slice(3))),
						);
						this.logDataReceived(
							'LogMessages.DataReceivedHighThroughput',
							undefined,
							new Uint8Array(combinedData).buffer,
						);
					} else {
						this.proteusHighThroughput.setFragmentsDataAtIndex(
							fragmentNumber - 1,
							data,
						);
					}
	...
	async formatdatatx(data: DataView): Promise<TxLogEntry[]> {
		...
		if (data.byteLength <= mtu - 1) {
			...
			// -3 is for the overhead of ProteusHeader.RF_HEADER_TYPE_FRAGDATA
			...
			while (remainingTotalLength > 0) {
				...
				fragmentArray.set(
					[
						ProteusHeader.RF_HEADER_TYPE_FRAGDATA,
						sequenceNumber,
						(fragmentsTotalCount & 0x0f) | ((fragmentsCount << 4) & 0xf0),
					],
					0,
				);
				...
				fragmentArray.set(
					Array.from(
						new Uint8Array(
							data.buffer.slice(
								startBytePayload,
								startBytePayload + fragmentLength,
							),
						),
					),
					3,
	...
	formatwritepinconfiguration(pins: GPIOPin[]): TxLogEntry {
			...
			switch (pin.getPinType()) {
				...
				case GPIOPinType.Input:
					data = data.concat([
						...
						pin.getPinID(),
						GPIOPinType.Input,
						pin.getPinConfigValue().getUint8(0),
					]);
					break;
				case GPIOPinType.Output:
					data = data.concat([
				...
				case GPIOPinType.PWM:
					data = data.concat([
						...
						pin.getPinID(),
						GPIOPinType.PWM,
						pin.getPinConfigValue().getUint8(1),
						pin.getPinConfigValue().getUint8(0),
						pin.getPinConfigValue().getUint8(2),
					]);
					break;
			}
		}
		...
		let fulldata = new Uint8Array(
			cmdheader.length + cmd.length + datauint8.length,
		);
		fulldata.set(cmdheader);
		fulldata.set(cmd, cmdheader.length);
	...
	private waitForCommandResponse(
		commandResponse: ProteusCommand,
		timeoutMs = 400,
	): Promise<ProteusCommand> {
